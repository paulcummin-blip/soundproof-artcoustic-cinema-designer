// Production P19 validation runner — processes one config at a time.
// Usage: node test/p19-production-validation-runner.mjs <room> <configIndex> <outputFile>
import { createServer } from "vite";
import { writeFileSync, readFileSync, existsSync } from "node:fs";

const roomArg = String(process.argv[2] || "B").toUpperCase();
const configIndex = parseInt(process.argv[3] || "0", 10);
const outputFile = String(process.argv[4] || "/tmp/p19-validation-partial.json");
...
const config = (quantity, comparison, sources, delaysMs, gainsDb, oldP19RawDb) => ({
  quantity, comparison, sources, tuning: { delaysMs, gainsDb }, oldP19RawDb, oldP19Grade: "FAIL",
});
...
const PHYSICS = Object.freeze({
  ...
  qStrategy: "ab_corrected", enableRewCoreReflections: false, roomDamping: 20,
  axialQ: 4, modalSourceReferenceMode: "existing", modalGainScalar: 1,
  modalDistanceBlend: 0.55, modalStorageMode: "none", propagationPhaseScale: 0,
  disableReflectionPhaseJitter: false, disableReflectionCoherenceWeight: false,
  disableLateField: true, disableModalPropagationPhase: true, mute68HzAxialMode: false,
  debugDisableModalContribution: false, rewParityFieldMode: "full_field",
  overrideConstantAxialQ: false, overrideAbsorptionAxialQ: false, debugMode200Multiplier: 1,
  reflectionGainScale: 1, rewParityModalMagnitudeScale: 1, modalCoherenceMode: "coherent",
  highOrderAxialScale: 1, rewModalBandwidthScale: 0.55, rewSourceCurveMode: "product",
  runtimeVectorCapture: false,
});

const fixed = room.configs[configIndex];
...
try {
  ...
  const sources = fixed.sources.map((position, index) => ({
    ...
    modelKey: MODEL_KEY, subwooferAmplifierPowerW: 1000, ...position, enabled: true,
    tuning: { gainDb: fixed.tuning.gainsDb[index], delayMs: fixed.tuning.delaysMs[index], polarity: 0, requestedOutputDb: TARGET_DB },
  }));

  const simulation = simulateAuthoritativeBassResponse({
    roomDims: room.roomDims, seatingPositions: room.seatingPositions,
    rspPosition: room.rspPosition, sources, physics: PHYSICS, qStrategyOverride: "ab_corrected",
  });
  const curves = buildAuthoritativeResponseCurves(simulation.seatResponses);
  const primaryIds = new Set(room.seatingPositions.filter((s) => s.isPrimary).map((s) => s.id));
  const perSeatRawCurves = curves.perSeatRawCurves.map((s) => ({ ...s, isPrimary: primaryIds.has(s.seatId) }));

  const fingerprint = `post-fix-p19:${room.id}:q${fixed.quantity}:${fixed.comparison}`;
  const pool = generateCandidatePool({
    rawCurve: curves.rspRawCurve, perSeatRawCurves, activeSubs: sources,
    usableLfHz: 22, transitionHz, correctionEndHz: 200,
    perSourceComplexTransfers: simulation.perSourceRspComplexTransfers,
    normalizedTransferFingerprint: fingerprint, calibrationFingerprint: fingerprint,
    selectedP14TargetDb: TARGET_DB, p14TargetBasis: "minimum", p14TargetLevel: TARGET_LEVEL,
    p18TargetBasis: "minimum", collectDiagnostics: false,
  });
  ...
  const authority = evaluateCanonicalBassAuthority({
    canonicalResult, activeSubs: sources, usableLfHz: 22,
    p14TargetBasis: "minimum", p18TargetBasis: "minimum", requestedLevel: TARGET_LEVEL,
  });
  if (!authority?.assessmentBandValid) throw new Error(`Invalid assessment band: ${authority?.limitation}`);

  const target = authority.practicalCalibrationTarget?.length
    ? authority.practicalCalibrationTarget : base.productionHouseCurveTarget;
  const diagnosticP19 = computeOfficialP19Assessment({
    rspPostEqCurve: base.finalPostEqCurve, canonicalTargetCurve: target,
    assessmentStartHz: 30, assessmentEndHz: 60,
  });
  const diagnosticP20 = computeOfficialP20Assessment({
    rspPostEqCurve: base.finalPostEqCurve, perSeatPostEqCurves: base.perSeatPostEqCurves,
    assessmentStartHz: 30, assessmentEndHz: 60,
  });
  const worstP20 = authority.perSeatP20Results?.find((s) => s.seatId === authority.worstP20SeatId);
  const gradeP19 = (level) => Number(level) > 0 ? `L${level}` : "FAIL";
  const gradeP20 = (level) => `L${level}`;

  const result = {
    room: room.name, quantity: fixed.quantity, comparison: fixed.comparison,
    modelKey: MODEL_KEY,
    positions: fixed.sources, delaysMs: fixed.tuning.delaysMs, levelTrimsDb: fixed.tuning.gainsDb,
    achievedP18CutoffHz: authority.achievedP18FrequencyHz, transitionHz,
    p19AssessmentBandHz: [authority.assessmentStartHz, authority.assessmentEndHz],
    correctedP19: {
      rawDb: authority.achievedP19VariationDb,
    ...
    beforeAfterP19: {
      oldRawDb: fixed.oldP19RawDb, oldGrade: fixed.oldP19Grade,
      correctedRawDb: authority.achievedP19VariationDb,
      correctedGrade: gradeP19(authority.achievedP19Level),
      deltaDb: authority.achievedP19VariationDb - fixed.oldP19RawDb,
      improvementDb: fixed.oldP19RawDb - authority.achievedP19VariationDb,
    },
    authority: {
  ...
  if (existsSync(outputFile)) {
    try { existing = JSON.parse(readFileSync(outputFile, 'utf8')); } catch { existing = []; }
  }
  existing[configIndex] = result;
  writeFileSync(outputFile, JSON.stringify(existing, null, 2));
  console.log(JSON.stringify({
    room: room.name, configIndex, comparison: fixed.comparison,
    p18: result.achievedP18CutoffHz, p19Raw: result.correctedP19.rawDb,
    p19Grade: result.correctedP19.grade, p19WorstHz: result.correctedP19.worstFrequencyHz,
    p20Raw: result.p20.rawDb, p20Grade: result.p20.grade,
    targetIdentity: result.authority.p19TargetIdentity,
  }, null, 2));
} finally {
  await server.close();
}