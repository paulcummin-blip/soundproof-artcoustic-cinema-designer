...
function tryFile(p) { try { if (existsSync(p) && statSync(p).isFile()) return p; } catch {} return null; }
function resolveCandidate(spec, fromDir) {
  if (spec.startsWith('@/')) { const rest = spec.slice(2); return tryFile('/app/src/' + rest) || tryFile('/app/src/' + rest + '.js') || tryFile('/app/src/' + rest + '.jsx'); }
  if (spec.startsWith('.')) { const base = join(fromDir, spec); return tryFile(base) || tryFile(base + '.js') || tryFile(base + '.jsx') || tryFile(base + '.mjs') || tryFile(join(base, 'index.js')); }
  if (spec.startsWith('/')) { return tryFile(spec) || tryFile(spec + '.js') || tryFile(spec + '.jsx'); }
  return null;
}
