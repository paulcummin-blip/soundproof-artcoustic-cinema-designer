...
function modalPressureContributionLocal(frequencyHz, modeFrequencyHz, qValue, combinedCoupling, modalSourceAmplitude, modeIndices) {
  ...
  // Below-resonance real-share increase test:
  // preserve the current helpful imaginary behaviour,
  // but increase only the real-part share so the modal vector
  // can oppose the pre-modal real axis more strongly at 40 Hz.
  const transferReal = -1.60 * (imagDen * (imagDen + 0.10)) / denominatorSq;
...
export function simulateBassResponseRewCore(roomDims, seatPos, sub, subProductCurve, options = {}) {
  ...
  const complexPressure = freqsHz.map((frequencyHz) => {
    ...
    imageSources.forEach((imageSource, reflectionIndex) => {
      ...
      const reflectionCoherenceWeight = 1.0;
      const imageRe = 0;
      const imageIm = 0;
