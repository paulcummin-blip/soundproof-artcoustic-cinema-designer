...
function RoomDesignerWithState() {
  ...
  useEffect(() => { if (didInitialLiveScreenSyncRef.current || resolvedProjectId || projectIdState || !appState?.isHydrated || Number(_seatingBlockOffset) !== 0) return; const sfp = Number(appState?.screenFrontPlaneM); if (!Number.isFinite(sfp) || sfp <= 0) return; const swm = Number(screenVisibleWidthInchesEffective) * 0.0254; if (!Number.isFinite(swm) || swm <= 0) return; const fixedY = sfp + distanceFor57_5FromWidth(swm); const cur = Number(appState?.mlpY_m); if (Number.isFinite(cur) && Math.abs(cur - fixedY) <= 0.005) { didInitialLiveScreenSyncRef.current = true; return; } const len = Number(stableDimensions?.length) || 6.0; const clamp = y => Math.max(0.40, Math.min(len - 0.40, y)); const ctrs = (buildRowCenters?.(fixedY, Number(_seatingRows)||1, Number(_rowSpacingM)||1.8, _mlpBasis)||[]).map(clamp); if (typeof appState?.setRowCentersM === 'function') appState.setRowCentersM(ctrs); if (typeof appState?.setMlpY_m === 'function') appState.setMlpY_m(Math.round(fixedY * 1000) / 1000); didInitialLiveScreenSyncRef.current = true; }, [appState?.isHydrated, appState?.screenFrontPlaneM, resolvedProjectId, projectIdState, _seatingBlockOffset, screenVisibleWidthInchesEffective, appState?.mlpY_m]);
          ...
          <div
            ...
            className="space-y-3">
              ...
              <div className="mb-3 text-xs px-3 py-2 rounded border border-amber-300 bg-amber-50 text-amber-800">
                  ...
                  {/* Aim Loudspeaker - Extracted component */}
                  <AimLoudspeakerPanel
                    ...
                    aimFrontWidesAtMLP={appState?.aimFrontWidesAtMLP}
                    setAimFrontWidesAtMLP={(v) => appState?.setAimFrontWidesAtMLP(v)}
                    aimSideSurroundsAtMLP={appState?.aimSideSurroundsAtMLP}
                    setAimSideSurroundsAtMLP={(v) => appState?.setAimSideSurroundsAtMLP(v)}
                    aimRearSurroundsAtMLP={appState?.aimRearSurroundsAtMLP}
                    setAimRearSurroundsAtMLP={(v) => appState?.setAimRearSurroundsAtMLP(v)}
