...
function ProjectCard({ project, onDelete }) {
  ...
  // Guard: ensure we only render for plain objects
  if (typeof project !== 'object') return null;

  // Map raw status -> branded label for badge
  ...
  const handleNavigate = (page) => {
    const url = `/${page}?project=${project.id}`;
...
+          </span>
      ...
      <CardFooter className="bg-[#F8F8F7] p-4 mt-auto border-t border-[#DCDBD6]">
        {/* Footer can be used for last updated date or other meta data later */}
      </CardFooter>
    </Card>
  );
}
