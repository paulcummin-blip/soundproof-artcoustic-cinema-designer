...
/**
 * Per-seat P19 assessment centering — analytical minimax normalisation.
 *
 * Per-seat P19 inherits the RSP's global trim, but each seat has a different
 * room transfer, leaving a seat-specific constant vertical offset in the
 * residual. This helper centers the residual curve analytically so P19
 * measures response-SHAPE deviation (minimax ±), not absolute-level error.
 *
 * centeringDb = -(maxResidual + minResidual) / 2
 * p19VariationDbRaw = (maxResidual - minResidual) / 2
 *
 * This is assessment normalisation ONLY — NOT a physical EQ filter, boost/cut
 * request, or installed calibration setting. It is never written back to
 * project tuning or shown as an Apply action. It is unconstrained by P14
 * headroom because it measures shape deviation, not operating level.
 *
 * Protected null points are excluded from max/min/worst-frequency exactly as
 * in the canonical P19 assessment — a narrow excluded null never influences
 * the centering result.
 *
 * @param {Array} residualCurve - residual points from evaluateP19AbsoluteTargetDeviation result
 *   Each point: { frequency, residualDb, protected, ... }
 * @returns {object|null} { variationDbRaw, centeringDb, worstFrequencyHz, maxResidualDb, minResidualDb, maxResidualHz, minResidualHz }
 */
export function centerResidualForP19Assessment(residualCurve) {
  if (!Array.isArray(residualCurve) || !residualCurve.length) return null;

  let maxResidualDb = -Infinity;
  let minResidualDb = Infinity;
  let maxResidualHz = null;
  let minResidualHz = null;

  for (const point of residualCurve) {
    if (point?.protected) continue;
    if (!Number.isFinite(point?.residualDb)) continue;
    const r = Number(point.residualDb);
    if (r > maxResidualDb) {
      maxResidualDb = r;
      maxResidualHz = point.frequency;
    }
    if (r < minResidualDb) {
      minResidualDb = r;
      minResidualHz = point.frequency;
    }
  }

  if (!Number.isFinite(maxResidualDb) || !Number.isFinite(minResidualDb)) return null;

  const centeringDb = -(maxResidualDb + minResidualDb) / 2;
  const variationDbRaw = (maxResidualDb - minResidualDb) / 2;

  // Worst frequency = argmax(abs(residual + centeringDb)) among non-protected.
  // After centering the extremes are symmetric; pick the largest absolute
  // centered residual so the marker tracks the genuine limiting deviation.
  let worstFrequencyHz = null;
  let worstAbsCentered = -Infinity;
  for (const point of residualCurve) {
    if (point?.protected) continue;
    if (!Number.isFinite(point?.residualDb)) continue;
    const centered = Number(point.residualDb) + centeringDb;
    const absCentered = Math.abs(centered);
    if (absCentered > worstAbsCentered) {
      worstAbsCentered = absCentered;
      worstFrequencyHz = point.frequency;
    }
  }
