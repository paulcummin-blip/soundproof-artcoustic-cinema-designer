...
import { houseCurveP19Level, runHouseCurveFitterFixtures, calculateHouseCurveEqCurve } from "@/components/utils/houseCurveFitter";
...
export function runRankingFixtures() {
  ...
  // to prove uniform seat metrics are finite for every profile and the comparator
  // is order-invariant with real candidates.
  {
    ...
    const rawCurve = freqs.map((f) => {
      ...
      return { frequency: f, spl: 85 + artcousticHouseCurveOffsetAt(f) + dev };
    });
    ...
    const standardEq = calculateDesignEqCurve(rawCurve, 35, activeSubs, {
      requestedSystemOutputDb: 85, targetAnchorDb: 85, targetToleranceDb: 2,
