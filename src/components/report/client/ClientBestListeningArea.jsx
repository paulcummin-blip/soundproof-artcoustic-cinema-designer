...
import { computeHaloRadiusPx, THREE_SEGMENT_LAYOUT, THREE_SEGMENT_LEGEND, PRIMARY_STROKE_WIDTH, buildRingSegmentPath } from "./seatMarkerGeometry";
...
// ── Seat matrix helpers ──
const MATRIX_PARAMS = [
  { key: "p4", label: "P4 Screen" },
  { key: "p6", label: "P6 Surround" },
  { key: "p10", label: "P10 Overhead" },
];

// Best-category rank: primary is strongest, not_assessed is weakest
const CATEGORY_BEST_RANK = { primary: 4, good: 3, acceptable: 2, improvement: 1, not_assessed: 0 };
...
export default function ClientBestListeningArea({
  ...
  // All seats use the SAME radius — different sizes would imply different
  // importance.  The radius never exceeds safeFraction × minSeatSpacing,
  // so adjacent halos always have a visible gap.
  ...
  return (
    <div style={containerStyle}>
      ...
      {showDrawing && (
        ...
        {/* Seat markers — compact spacing-aware three-segment halo.
             TOP = P4 Screen, LOWER-LEFT = P6 Surround, LOWER-RIGHT = P10 Overhead.
             Primary seats get an additional bold dark outer keyline. */}
        {seats.map((seat) => {
          const sp = toPx(seat.x, seat.y);
          const segments = THREE_SEGMENT_LAYOUT.map((seg) => ({
            key: seg.key,
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 14 }}>
          {THREE_SEGMENT_LEGEND.map((entry) => {
            const seg = THREE_SEGMENT_LAYOUT.find((s) => s.key === entry.key);
            return (
