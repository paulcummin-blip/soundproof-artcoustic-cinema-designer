 ...
 * buildAdiAssessment() and gets the same assessment. Only the presentation
 * changes. Never the wording.
 *
 * AUDIENCE AWARENESS:
 *   The builder accepts an optional `audience` parameter ("technical" |
 *   "dealer" | "client"). The engineering truth never changes — only the
 *   level of explanation changes. Currently only "technical" is implemented;
 *   the structure is in place so dealer/client profiles can be added later
 *   without rewriting the engineering content.
 *
...
/**
 * Extract the engineering summary from the published authority.
 *
 * The published authority is the full design-review handoff snapshot. The
 * engineering summary is one structured field within it. ADI reads it
 * directly from the authority — it does not receive it as a separate input.
 */
 ...
 *
 * @param {Object} publishedAuthority — the full design-review handoff
 *   snapshot published by the Room Designer. ADI reads the engineering
 *   summary field directly from this authority.
 * @param {Object} [options]
 * @param {string} [options.audience="technical"] — target audience profile.
 *   "technical" | "dealer" | "client". The engineering truth never changes;
 *   only the level of explanation changes. Currently only "technical" is
 *   implemented; the structure is in place for future audience profiles.
