...
React.useEffect(() => {
  ...
  const { BaffleAndScreen, screenPlaneY, screenCenterX_m, visibleWidthM } = useMemo(() => {
    ...
    // EXPORT: only lock to saved screenFrontPlaneM when screenPlaneMode is FIXED
          ...
          >
            ...
            {exportMode !== 'clean' && overlaysForRendering?.ROOM_DIMS && (
              <g data-layer="room-dimensions">
                ...
                {/* --- Front wall labels: RSP + row distances (only when ROOM_DIMS is on) --- */}
                {(() => {
                  // Safety: if we can't place things reliably, do nothing.
                  if (!roomRect || typeof toPx !== 'function') return null;

                  const frontWallY = (roomRect?.y ?? 0);

                  // 1) RSP label aligned to MLP X, placed on/near the front wall
                  let rspX = null;
                  if (Number.isFinite(mlpDotX_m)) {
                    const [rspPxX] = toPx(mlpDotX_m, 0);
                    rspX = rspPxX;
                  }

                  // 2) Row labels: group seats by y (distance from front wall), pick furthest-right seat per row
                  // We'll bin y to 1cm so seats that are "same row" stay together.
                  const seats = Array.isArray(seatingPositions) ? seatingPositions : [];
                  const rows = new Map(); // key -> { yM, xMmax }
                  for (const seat of seats) {
                    const xM = Number(seat?.x ?? seat?.position?.x ?? 0);
                    const yMraw = Number(seat?.y ?? seat?.position?.y ?? 0);
                    const yM = typeof clampMlpY === 'function' ? clampMlpY(yMraw) : yMraw;

                    if (!Number.isFinite(xM) || !Number.isFinite(yM)) continue;

                    const key = Math.round(yM * 100); // 0.01m bins
                    const existing = rows.get(key);
                    if (!existing || xM > existing.xMmax) {
                      rows.set(key, { yM, xMmax: xM });
                    }
                  }
