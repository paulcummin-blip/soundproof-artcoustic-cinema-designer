 ...
 *
 * Diagnostic-only. Tests 12 combinations of Q scale × tangential scale.
 * Q scales: 0.80, 0.70, 0.60, 0.50
 * Tangential scales: 0.80, 0.70, 0.60
 * Axial = 1.0, oblique = 1.0 fixed.
...
const Q_SCALES        = [0.80, 0.70, 0.60, 0.50];
const TANG_SCALES     = [0.80, 0.70, 0.60];
...
function runSim(roomDims, seat, sub, surfaceAbsorption, activeSettings, qScale, tangScale) {
...
const thStyle = {
  ...
  background: '#fdf4ff', borderBottom: '2px solid #d8b4fe', color: '#6b21a8',
  whiteSpace: 'nowrap',
};
const tdStyle = { textAlign: 'right', padding: '2px 6px', fontSize: 10, fontFamily: 'monospace' };

export default function RewParityQTangentialSweep({
  ...
  const runSweep = useCallback(async () => {
    ...
    for (const qScale of Q_SCALES) {
      for (const tangScale of TANG_SCALES) {
        const series  = runSim(roomDims, seat, sub, surfaceAbsorption, activeSettings, qScale, tangScale);
        const metrics = series ? computeMetrics(series) : null;
        const bands   = BAND_DEFS.map(({ lo, hi }) =>
          series ? computeBandMAE(series, lo, hi) : null
        );
        computed.push({ qScale, tangScale, metrics, bands });
      }
    }
  ...
  return (
    <div style={{
      ...
      background: '#fdf4ff', padding: '10px 12px',
    }}>
      <div style={{ fontWeight: 700, color: '#6b21a8', fontSize: 11, fontFamily: 'monospace', marginBottom: 4 }}>
        Q × Tangential Combined Sweep
        <span style={{ fontWeight: 400, color: '#a855f7', marginLeft: 8, fontSize: 10 }}>
          diagnostic only · axial = 1.0, oblique = 1.0 fixed · 12 combinations · does not affect live graph
        </span>
      </div>
      <div style={{ fontSize: 9, color: '#7e22ce', fontFamily: 'monospace', marginBottom: 6 }}>
        Q scales: {Q_SCALES.join(', ')} &nbsp;·&nbsp; Tangential scales: {TANG_SCALES.join(', ')}
      </div>
      ...
      >
        {running ? 'Running…' : rows ? 'Re-run' : 'Run Q × Tang sweep'}
      </button>
      ...
      {rows && (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 600 }}>
            ...
            <tbody>
              {rows.map((row, i) => {
                const { qScale, tangScale, metrics, bands, isBest } = row;
                return (
                  <tr
                    key={i}
                    style={{
                      borderBottom: '1px solid #ede9fe',
                      background: isBest ? '#f3e8ff' : i % 2 === 0 ? '#fdf4ff' : undefined,
                    }}
                  >
                    <td style={{ ...tdStyle, textAlign: 'center', fontWeight: isBest ? 700 : 400, color: isBest ? '#6b21a8' : '#374151' }}>
                      {isBest ? `⭐ ${fmt(qScale, 2)}` : fmt(qScale, 2)}
                    </td>
                    <td style={{ ...tdStyle, textAlign: 'center', fontWeight: isBest ? 700 : 400, color: isBest ? '#6b21a8' : '#374151' }}>
                      {fmt(tangScale, 2)}
                    </td>
                    <td style={{ ...tdStyle, fontWeight: isBest ? 700 : 400, color: isBest ? '#6b21a8' : '#374151' }}>
                      {fmt(metrics?.mae, 3)}
                    </td>
                    <td style={{
                      ...tdStyle,
