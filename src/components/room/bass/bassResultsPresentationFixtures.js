...
export function runBassResultsPresentationFixtures() {
  ...
  { const f = formatBassResults(ready());   check("4. Ready values format correctly", f.pills.p14.text === "Estimated LFE Capability L1 · 115 dBC — Minimum target" && f.pills.p18.text === "Bass Extension L2 · 23 Hz" && f.pills.p19.text === "Seat Consistency L1 · ±4 dB" && f.pills.p20.text === "Seat Consistency L2 · ±3 dB"); }
  check("5. FAIL retains value", formatBassResults(ready({ level: 0 })).pills.p14.text === "P14 FAIL · 115 dBC — Minimum target");
  check("6. Single-seat P20 is N/A", formatBassResults(ready({ seats: 1 })).pills.p20.text === "Seat Consistency N/A");
  check("7. Multi-seat P20 uses contract authority", formatBassResults(ready()).pills.p20.text === "Seat Consistency L2 · ±3 dB");
  ...
  check("29. Seat selection preserves official P19 RSP", formatBassResults(ready(), Date.now(), "s1").pills.p19.text === "Seat Consistency L1 · ±4 dB");
  check("30. P20 formatting uses authoritative deviation", formatBassParameterValue("p20", 3.8) === "±3 dB");
  { const r = ready(); Object.assign(r.job, { status: "error", resultFingerprint: null });   check("31. P20 job error remains explicit", formatBassResults(r).pills.p20.text === "Seat Consistency error"); }
