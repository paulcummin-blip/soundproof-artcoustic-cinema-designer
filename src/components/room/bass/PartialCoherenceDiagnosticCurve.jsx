...
function buildChartData(coherentSeries, partialSeries) {
...
function DiagnosticTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: '#ffffff', border: '1px solid #bae6fd', borderRadius: 6, padding: '8px 10px', fontSize: 10, fontFamily: 'monospace', color: '#0f172a', boxShadow: '0 6px 18px rgba(15, 23, 42, 0.12)' }}>
      ...
      <div>Partial coherence: {Number.isFinite(partial) ? `${partial.toFixed(2)} dB` : '—'}</div>
      <div>Difference: {Number.isFinite(diff) ? `${diff >= 0 ? '+' : ''}${diff.toFixed(2)} dB` : '—'}</div>
      <div style={{ marginTop: 4, color: '#0369a1', fontStyle: 'italic' }}>Diagnostic only — not used for scoring</div>
    </div>
  );
}

export default function PartialCoherenceDiagnosticCurve({ b44Series, partialCoherenceDiagnosticSeries }) {
  const chartData = React.useMemo(() => {
    return buildChartData(
      normaliseCoherentSeries(b44Series),
      normalisePartialSeries(partialCoherenceDiagnosticSeries)
    );
  }, [b44Series, partialCoherenceDiagnosticSeries]);

  const hasPartialCurve = chartData.some((point) => Number.isFinite(point.partialCoherenceDb));
    ...
    <div style={{ marginTop: 10, padding: '8px 10px', borderRadius: 6, background: '#ecfeff', border: '1px solid #67e8f9' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', marginBottom: 4 }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: '#0e7490' }}>
          Partial coherence diagnostic — not used for scoring
        </div>
        <div style={{ fontSize: 9, color: '#64748b', fontFamily: 'monospace' }}>
          Overlay only · coherent benchmark curve unchanged
        </div>
      </div>
      <div style={{ fontSize: 10, color: '#64748b', marginBottom: 8 }}>
        Shows the existing coherent final curve against a diagnostic-only partial modal/pre-modal coherence curve.
      </div>
      <div style={{ height: 260 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 20 }}>
            ...
            />
            <Line
              name="Partial coherence diagnostic — not used for scoring"
              type="monotone"
