...
export default function DesignEqLimitStatus({ enabled, onChange }) {
  ...
  const targetLevel = Math.max(1, Math.min(4, Number(splConfig?.selectedP14Level ?? splConfig?.bassTargetLevel) || 4));
  ...
  return <div className="flex flex-col gap-1 text-xs text-muted-foreground">
    ...
    <span>Target: RP22 {targetBasis} L{targetLevel} House Curve</span>
