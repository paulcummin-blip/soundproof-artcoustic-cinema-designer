  ...
  return <details className="mt-3 rounded border border-emerald-300 bg-emerald-50 p-3 text-xs" open>
    ...
    <div className="mt-2 overflow-x-auto">
      <table className="min-w-[820px] text-right font-mono text-[10px] text-slate-700">
        <thead className="border-b border-slate-300 text-slate-500"><tr>{["Candidate", "Target SPL", "P14 result", "P18 result", "P19 result", "All ≥ L1", "Selected"].map((label) => <th className="px-2 py-1" key={label}>{label}</th>)}</tr></thead>
        <tbody>{result.candidates.map((candidate) => {
          ...
          return <React.Fragment key={candidate.requestedP14Level}>
            <tr className="border-b border-slate-200"><td className="px-2 py-1 font-semibold">{candidate.requestedP14Level}</td><td className="px-2 py-1">{candidate.requestedTargetSpl} dB</td><td className="px-2 py-1">{level(candidate.achievedP14Level)} · {fmt(candidate.achievedP14Db, " dB")}</td><td className="px-2 py-1">{level(candidate.achievedP18Level)} · {fmt(candidate.achievedP18FrequencyHz, " Hz")}</td><td className="px-2 py-1">{level(candidate.achievedP19Level)} · ±{fmt(candidate.achievedP19VariationDb, " dB")}</td><td className="px-2 py-1">{candidate.allAtLeastL1 ? "Yes" : "No"}</td><td className="px-2 py-1">{selected ? "Yes" : "—"}</td></tr>
            {!candidate.allAtLeastL1 && <tr className="border-b border-slate-200 bg-amber-50 text-left text-amber-900"><td className="px-2 py-1" colSpan="7">{priorityMode === "balanced" ? `Rejected: ${candidate.rejectionReason}` : `Below Level 1: ${candidate.rejectionReason}`} · Capability-limited: {candidate.capabilityLimitedFrequencies.length ? `${candidate.capabilityLimitedFrequencies.map((hz) => Math.round(hz)).join(", ")} Hz` : "none"}</td></tr>}
          </React.Fragment>;
        })}</tbody>
      </table>
    </div>
