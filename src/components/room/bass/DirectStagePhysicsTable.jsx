...
export default function DirectStagePhysicsTable({ response, subs = [], rspPosition = null }) {
  const physics = response?.debugPhysics;
  const enabledSubs = Array.isArray(subs) ? subs.filter((sub) => sub?.enabled !== false) : [];
  const formatCoordinate = (value) => Number.isFinite(value) ? value.toFixed(3) : "—";
  ...
  return (
    <div style={{ border: "1px solid #CBD5E1", borderRadius: 6, background: "#FFFFFF", padding: "8px 10px", fontSize: 10, fontFamily: "monospace", marginBottom: 8 }}>
      ...
      <div style={{ marginBottom: 3 }}>Enabled subs: {enabledSubs.length || "—"} | IDs: {enabledSubs.map((sub) => sub.id || "—").join(", ") || "—"} | Models: {enabledSubs.map((sub) => sub.modelKey || "—").join(", ") || "—"}</div>
      <div style={{ marginBottom: 6 }}>RSP: x={formatCoordinate(rspPosition?.x)}, y={formatCoordinate(rspPosition?.y)}, z={formatCoordinate(rspPosition?.z)}</div>
