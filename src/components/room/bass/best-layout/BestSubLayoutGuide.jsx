...
export default function BestSubLayoutGuide({ roomDims, seatingPositions, rspPosition, sourceHeights }) {
  const live = useBestSubLayoutLiveInputs();
  const canonical = useMemo(() => canonicalizeNormalizedRoomInputs({ roomDims: live?.roomDims ?? roomDims, seatingPositions: live?.seatingPositions ?? seatingPositions, rspPosition: live?.rspPosition ?? rspPosition }), [live, roomDims, seatingPositions, rspPosition]);
  const recommendation = useBestSubLayoutRecommendations({ ...canonical, physicsOptions: live?.physicsOptions, sourceHeights: live?.sourceHeights ?? sourceHeights });
  ...
  return (
    <div className="mt-4 rounded-lg border border-[#E7E4DF] bg-white/70 px-4 py-4" data-layout-candidates={recommendation.result?.candidateCount ?? 0} data-layout-cards={items.slice(0, 3).length}>
      ...
      {recommendation.status === "idle" && <p className="mt-3 text-xs text-[#625143]">Open Bass Simulation to load the live room physics, dimensions and seating.</p>}
