...
// (positions, tuning, models, enabled), selected P14 target, and physics/engine
 ...
 *
 * Uses computeCalibrationFingerprint (the same authority used by the production
 * bass analysis path) with BASS_NORMALIZED_PHYSICS_DEFAULTS + Stage 2 overrides
 * so the fingerprint covers physics/engine version.
 *
 * @param {object} params - V2 engine inputs
 * @returns {string} calibration fingerprint (e.g. "cal:v7:hashhashhashhash")
 */
export function computeV2DesignFingerprint(params) {
  ...
  return computeCalibrationFingerprint({
    roomDims,
 ...
 * used directly as the Current control (BLOCKER 3).
 *
 * The authority is non-stale when:
 *   1. authoritative === true (publication verified)
 *   2. contract exists with per-seat P19/P20 data
 *   3. the V2 start fingerprint matches the authority's currentFingerprint
 *
 * If the fingerprints don't match (because V2 uses slightly different defaults),
 * this returns false — the caller falls back to canonical recalculation, which
 * is always safe.
 *
 * @param {object} currentAuthority - completedBassAuthority from shared results
 * @param {string} v2StartFingerprint - V2 design fingerprint at start
...
export function isCurrentAuthorityNonStale(currentAuthority, v2StartFingerprint) {
  ...
  if (perSeatP19.length === 0 || perSeatP20.length === 0) return false;

  // Check fingerprint match — if the V2 fingerprint matches the authority's
  // currentFingerprint, the authority is non-stale. If they don't match,
  // fall back to recalculation (safe).
  if (v2StartFingerprint && currentAuthority.currentFingerprint) {
    return v2StartFingerprint === currentAuthority.currentFingerprint;
  }
