...
// Local helpers (inlined from SpeakerPlacement to avoid import cycles)
...
function ensureLcrWhenSelectingModel(modelLabel, dimensions, setSpeakers) {
  setSpeakers(prev => {
    ...
    const midX = roomW / 2;
    const seeded = [
      { ...FL, role: "FL", id: FL.id || "FL-1", model: modelLabel, position: FL.position || { x: midX - spread, y: defaultY, z: defaultZ }, rotation: FL.rotation || { x: 0, y: 0, z: 0 } },
      { ...FC, role: "FC", id: FC.id || "FC-1", model: modelLabel, position: FC.position || { x: midX, y: defaultY, z: defaultZ }, rotation: FC.rotation || { x: 0, y: 0, z: 0 } },
      { ...FR, role: "FR", id: FR.id || "FR-1", model: modelLabel, position: FR.position || { x: midX + spread, y: defaultY, z: defaultZ }, rotation: FR.rotation || { x: 0, y: 0, z: 0 } },
    ];
...
/**
 * LCRPanel — LCR model selector, amplifier power, P12 mode toggle, and P12 level pill.
 *
 * Props:
 *   setSpeakers, dimensions, lcrAimMode, onChangeLcrAimMode, lcrAngleDeg,
 *   mlpPoint, disabled, allSeatSplMetrics
 *
 *   updateGlobalSpl (optional) — if provided, overrides appState.updateGlobalSpl.
 *   This allows RoomDesigner to intercept radiationMode changes for immediate project sync.
 */
export default function LCRPanel({
  setSpeakers,
  dimensions,
  lcrAimMode,
  onChangeLcrAimMode,
  ...
  disabled,
  allSeatSplMetrics,
  updateGlobalSpl: updateGlobalSplProp,
}) {
  const appState = useAppState();
  const { speakerSystem, splConfig = {}, updateGlobalSpl: updateGlobalSplFromState, seatingPositions } = appState || {};

  // Allow prop to override appState version (used by RoomDesigner for project sync)
  const updateGlobalSpl = updateGlobalSplProp || updateGlobalSplFromState;
  ...
  const byRole = useMemo(() => buildRoleMap(speakerSystem?.placedSpeakers || []), [speakerSystem?.placedSpeakers]);
  ...
  return (
    <div className="space-y-2 p-2">
      ...
      {(() => {
        ...
        return <RP22LevelPill level={level} label="RP22 P12" />;
      })()}
    </div>
  );
}