...
// Model may be blank early for surround bed roles; RV will resolve a fallback model later.
const MODEL_OPTIONAL_ROLES = new Set([
  // Canonical surround bed roles
  "SL", "SR", "SBL", "SBR", "LW", "RW",
  // Common aliases that appear in this project
  "LR", "RR", "LRS", "RRS", "FWL", "FWR",
  // Safety net if generated by positioning logic
  "LBS", "RBS",
]);

const canonRole = (role) => {
  // Keep it dead-safe: no imports required, no crashes
...
export const isRenderableSpeaker = (speaker) => {
  ...
  // Most speakers require a selected model to render.
  // Surround bed roles are allowed to render with a blank model because
  // RoomVisualisation resolves a fallback model for icon sizing later.
  const role = canonRole(speaker?.role);

  const m = speaker.model;
  const ms = String(m ?? "").trim().toLowerCase();
  const modelInvalid = !ms || ms === "off" || ms === "none";

  if (modelInvalid && !MODEL_OPTIONAL_ROLES.has(role)) return false;
