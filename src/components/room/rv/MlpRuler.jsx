...
/**
 * MlpRuler – renders the MLP position ruler overlay on the room plan SVG.
 * Shows distances from MLP to all four walls plus a secondary front-wall depth ruler.
 */
export default function MlpRuler({
  ...
  return (
    <g data-layer="mlp-ruler" pointerEvents="none">
      ...
      <line
        x1={(roomRect?.x ?? 0)} y1={mlpY_px}
        x2={(roomRect?.x ?? 0) + (roomRect?.width ?? 0)} y2={mlpY_px}
        stroke={rulerColor} strokeWidth={rulerStroke} opacity={0.6}
      />
      ...
      <text x={((roomRect?.x ?? 0) + mlpX_px) / 2} y={mlpY_px - labelOffset}
        textAnchor="middle" fontSize={fontSize} fill={rulerColor} fontFamily={fontFamily}>
        {distLeftWall.toFixed(2)}m
      </text>
      <text x={(mlpX_px + (roomRect?.x ?? 0) + (roomRect?.width ?? 0)) / 2} y={mlpY_px - labelOffset}
        textAnchor="middle" fontSize={fontSize} fill={rulerColor} fontFamily={fontFamily}>
      ...
      <line
        x1={mlpX_px} y1={screenY_px}
        x2={mlpX_px} y2={(roomRect?.y ?? 0) + (roomRect?.height ?? 0)}
        stroke={rulerColor} strokeWidth={rulerStroke} opacity={0.6}
      />
      ...
      <text x={mlpX_px - labelOffset} y={(screenY_px + mlpY_px) / 2}
        textAnchor="middle" fontSize={fontSize} fill={rulerColor} fontFamily={fontFamily}
        transform={`rotate(-90 ${mlpX_px - labelOffset} ${(screenY_px + mlpY_px) / 2})`}>
        {distScreen.toFixed(2)}m
      </text>
      <text x={mlpX_px + labelOffset} y={(mlpY_px + (roomRect?.y ?? 0) + (roomRect?.height ?? 0)) / 2}
        textAnchor="middle" fontSize={fontSize} fill={rulerColor} fontFamily={fontFamily}
      ...
      <defs>
        <marker id="mlp-depth-arrow" viewBox="0 0 10 10" refX="5" refY="5"
          markerWidth="4" markerHeight="4" orient="auto">
          <path d="M 0 0 L 10 5 L 0 10 z" fill={rulerColor} />
        </marker>
      </defs>
      <line
        x1={secondaryRulerX_px} y1={(roomRect?.y ?? 0)}
        x2={secondaryRulerX_px} y2={mlpY_px}
        stroke={rulerColor} strokeWidth={rulerStroke} opacity={0.6}
        markerStart="url(#mlp-depth-arrow)" markerEnd="url(#mlp-depth-arrow)"
      />
      <text x={secondaryRulerX_px + labelOffset} y={((roomRect?.y ?? 0) + mlpY_px) / 2}
        textAnchor="middle" fontSize={fontSize} fill={rulerColor} fontFamily={fontFamily}
