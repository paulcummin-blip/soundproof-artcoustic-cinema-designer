...
export default function P14LevelPill({ showSpl = false }) {
  const p14 = useLiveP14Level();

  if (!p14.hasData) {
    return (
      <span title="P14 post-EQ LFE SPL capability at RSP">
        <RP22GradingPill level="—">P14 —</RP22GradingPill>
      </span>
    );
  }

  const levelForPill = p14.level ?? 'FAIL';
  const label = (showSpl && Number.isFinite(p14.valueDb))
    ? `P14: ${p14.valueDb.toFixed(1)} dB post-EQ at RSP`
    : `P14 ${levelForPill}`;

  return (
    <span title="P14 post-EQ LFE SPL capability at RSP">
      <RP22GradingPill level={levelForPill}>{label}</RP22GradingPill>
    </span>
  );
}